...
class Generator:
        ...
        for i in range(1, 13):
            ...
            rows.append(
                dict(
                    ...
                    region=region,
                    opened_date=(
                        WINDOW_START - timedelta(days=self.rng.randint(400, 2000))
                    ).isoformat(),
                    is_active=True,
                )
            )
            ...
            rows.append(
                dict(
                    ...
                    status=self.rng.choices(
                        ["ACTIVE", "DISCONTINUED", "PENDING"], weights=[88, 8, 4]
                    )[0],
    ...
    def orders(
        ...
        for i in range(1, self.cfg["orders"] + 1):
            ...
            status = self.rng.choices(ORDER_STATUS, weights=[4, 6, 12, 68, 6, 4])[0]
            ...
            order = dict(
                ...
                shipping_status=(
                    "DELIVERED" if status == "DELIVERED" else self.rng.choice(SHIPPING_STATUS)
                ),
            ...
            items = []
            for line in range(
                1, self.rng.choices([1, 2, 3, 4, 5], weights=[42, 27, 17, 9, 5])[0] + 1
            ):
                ...
                gross = unit * qty
                disc = (
                    money(float(gross) * self.rng.uniform(0.05, 0.30))
    ...
    def inventory(self, products: list[dict], stores: list[dict], days: int = 30) -> Iterator[dict]:
        ...
        for n, o in enumerate(orders, 1):
            ...
            ships.append(
                dict(
                    ...
                    delivered_at=(
                        delivered.isoformat() if o["order_status"] == "DELIVERED" else None
                    ),
            ...
            if self.rng.random() < 0.07:
                ...
                rets.append(
                    dict(
                        return_id=f"RET-{n:09d}",
                        order_id=o["order_id"],
                        product_id=None,
                        return_reason=self.rng.choice(RETURN_REASONS),
    ...
    def events(
        self, customers: list[dict], products: list[dict], orders: list[dict]
    ) -> Iterator[dict]:
        ...
        while made < target:
            ...
            for step in funnel:
                ...
                yield dict(
                    ...
                    order_id=(
                        order_by_cust.get(cid, {}).get("order_id")
                        if step in ("order_created", "payment_completed") and cid
                        else None
                    ),
                    channel=channel,
                    device_type=device,
